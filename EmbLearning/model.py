from utils.batch_utils import Batch_Loader, Batch_Loader_with_groundings
...
class Model(object):
    ...
    def add_placeholders(self):
        ...
        self.weight = tf.placeholder(tf.float32, [1], name="weight")
    ...
    def add_training_op(self):
        ...
        self.grads_and_vars = [(ClipIfNotNone(g), v) for g, v in self.grads_and_vars]
    ...
    def fit(self, sess, train_triples, valid_triples=None, scorer=None, groundings=None):
        ...
        if groundings is not None and len(groundings[1])>0:
            train_batch_loader = Batch_Loader_with_groundings(
                groundings, train_triples, self.n_entities, self.n_relations, batch_size=self.batch_size,
                neg_ratio=self.neg_ratio, contiguous_sampling=self.contiguous_sampling)
        else:
            '''
            ...
            '''
            train_batch_loader = Batch_Loader(
                train_triples, self.n_entities, batch_size=self.batch_size, neg_ratio=self.neg_ratio,
                contiguous_sampling=self.contiguous_sampling)
